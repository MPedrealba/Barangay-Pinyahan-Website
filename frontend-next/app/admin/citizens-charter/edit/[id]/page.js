...
export default function EditCitizensCharterPage({ params }) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setFormData({
            ...
            office_division: c.office_division || 'Administrative Division',
            classification: c.classification || 'Simple',
            transaction_type: c.transaction_type || 'G2C',
            who_may_avail: c.who_may_avail || '',
            requirements:
              c.requirements && c.requirements.length > 0
                ? c.requirements.map((r) => ({
                    name: r.name || r.requirement_name || '',
            ...
            steps:
              c.steps && c.steps.length > 0
                ? c.steps.map((s) => ({
                    ...
                    agency_action: s.agency_action || s.action_taken || '',
                    fees: s.fees || 'None',
          ...
          showToast('error', 'Charter service not found.');
        }
      } catch (err) {
        console.error('Fetch error:', err);
        showToast('error', 'Failed to load charter details.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="p-8 max-w-5xl mx-auto flex items-center justify-center py-32 text-gray-400 text-sm gap-2">
        <i className="fas fa-spinner fa-spin text-2xl text-[#0056b3]" />
        <span>Loading Citizen&apos;s Charter...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">
            ...
            <span>/</span>
            <span className="text-gray-700">Edit Charter</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">
            Edit Citizen&apos;s Charter
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Update ARTA-compliant service information and processing steps.
          </p>
        </div>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  <span>Updating Charter...</span>
                </>
              ) : (
                <>
                  <i className="fas fa-save text-sm" />
                  <span>Update Citizen&apos;s Charter</span>
                </>
              )}
            </button>
          </div>
